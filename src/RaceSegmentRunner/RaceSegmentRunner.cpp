...
double RaceSegmentRunner::calculate_resistive_force(const RouteSegment& route_segment,
                                                    const WeatherDataPoint& weather_data, double speed) const {
...
double RaceSegmentRunner::calculate_power_out(const RouteSegment& route_segment, const WeatherDataPoint& weather_data,
                                              double speed) const {
...
double RaceSegmentRunner::calculate_power_in([[maybe_unused]] const RouteSegment& route_segment,
                                             const WeatherDataPoint& weather_data) const {
    return car.array.power_in(weather_data.irradiance);
}

std::optional<double> RaceSegmentRunner::calculate_power_net(const RouteSegment& route_segment,
                                                             const WeatherDataPoint& weather_data,
                                                             double state_of_charge, double speed) const {
    double net_power_demanded
      = calculate_power_in(route_segment, weather_data) - calculate_power_out(route_segment, weather_data, speed);
