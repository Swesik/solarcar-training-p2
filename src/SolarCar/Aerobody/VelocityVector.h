...
public:
    ...
    double get_north_south() const { return north_south; }

    double get_east_west() const { return east_west; }

    /// @brief Calculate the magnitude of the vector
    double get_magnitude() const { return std::sqrt(north_south * north_south + east_west * east_west); }

    /// @brief Calculates the heading
    /// @note recall that 0 radians is due north, and π/2 radians is due east, and the range is
    /// [0, 2π]
    /// @note returns 0 if the vector has magnitude zero
    double get_heading() const;

    /// @brief Calculate the angle, in radians, between the two vectors
    ///
    /// @return (radians) the angle between two vectors
    /// @note positive if { other } is clockwise of this vector
    ...
    VelocityVector operator+(const VelocityVector& other) const {
        return VelocityVector(north_south + other.north_south, east_west + other.east_west);
    }
    ...
    VelocityVector operator-() const { return VelocityVector(-north_south, -east_west); }
